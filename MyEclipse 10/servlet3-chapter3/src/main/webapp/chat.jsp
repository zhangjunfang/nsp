...
<body>
	...
	<div id="user-list">
		<ul>
			<%
				Object  login= request.getAttribute("loginUsers");
			            if(null!=login){
			                Set<String> loginUsers = (Set<String>) login;
	...
	我自己：
	<span id="username">${param.username}</span>
	<br /> 发送给：
	<input type='text' id="receiver">(不填代表所有人)<br />
		消&nbsp;&nbsp;息：<textarea id="msg" cols="20" rows="5"></textarea><br />
		<input id="send-btn" type="button" value="发送" /> <script
			src="${pageContext.request.contextPath}/static/jquery.1.10.2.js"></script>
			...
			$(function() {
				...
				var chat = {
					...
					publish : function(username, msg) {
						...
						msgBox.append($("<div></div>").addClass("msg")
								.html(msg));
						msgBox.animate({
							scrollTop : msgBox.scrollTop() + msgBox.height()
						}, 300);
					},
					callback : function(data) {
						...
						case "logout":
							...
							break;
						case "msg":
							chat.publish(data.username, data.username + ":"
									+ data.msg);
							break;
						default:
							//ignore
					...
					longPolling : function(url, callback) {
						$.ajax({
							...
							success : function(data, status, request) {
								...
								request = null;
								setTimeout(function() {
									chat.longPolling(pollUrl, callback);
								}, 10);
							},
				...
				$("#send-btn")
						.click(
								function() {
									...
									$
											.get("${pageContext.request.contextPath}/chat?command=send&receiver="
													+ receiver + "&msg=" + msg);
								});
